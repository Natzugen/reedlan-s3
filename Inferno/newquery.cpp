...
BOOL CQuery::Connect(TCHAR * lpszDNS, TCHAR * lpszUser, TCHAR * lpszPassword)
{
	...
	if ( Result != SQL_SUCCESS && Result != SQL_SUCCESS_WITH_INFO )
	{
		...
		{
			//cout << "SQLSTATE:" << SqlState << ", Diagnosis:" << SQLMsgError << endl;
//			cLog.ConsoleOutput(DARKRED,"SQLSTATE:%s, Diagnosis:%s",SqlState,SQLMsgError);
		}
...
BOOL CQuery::ExecQuery(TCHAR * lpszStatement, ...)
{
	...
	//cout << szStatement << endl;
//	cLog.ConsoleOutput(DARKRED,"%s",szStatement);
	...
	if ( this->m_ColCount >= MAX_COLUMNS-1 )
	{
		...
		//cout << "[CQuery] ColCount >= MAX_COLUMNS-1" << endl;
//		cLog.ConsoleOutput(DARKRED,"[CQuery] ColCount >= MAX_COLUMNS-1");
		return FALSE;
	}
